...
class ExcelInterface:
    ...
    def read_payor_and_fee_schedule(self):
    ...
    def read_fee_schedule_rates(self):
        self.fee_schedule_rates = self.excel.read_worksheet_as_table(self.get_real_sheet_name('Fee Schedule Rates'), True)
        for row in range(len(self.fee_schedule_rates)):
            index_of_cd_num: int = self.fee_schedule_rates.columns.index('CdNum')
            self.fee_schedule_rates.set_cell(row, index_of_cd_num, str(self.fee_schedule_rates[row, index_of_cd_num]).strip('.0'))
            for col in range(len(self.fee_schedule_rates[row])):
                temp_value = self.fee_schedule_rates[row, col]
                if not temp_value:
                    continue
                if type(temp_value) == str:
                    temp_value: str = temp_value.strip()
                    if temp_value.lower() == 'HS Diploma'.lower():
                        temp_value = 'High School Diploma'
                    elif temp_value.lower() == 'Doctorate'.lower():
                        temp_value = 'Medical Doctorate'
                self.fee_schedule_rates.set_cell(row, col, temp_value)

    def read_bcba_credential(self):
    ...
    def read_fee_schedule_detail(self):
        self.fee_schedule_table, self.fee_schedule_header_row, self.fee_schedule_columns_index = \
            self.read_sheet_and_find_headers('Fee Schedule detail',
                                             ['FeeSched', 'CdNum', 'CdDscrpt', 'Can Overlap', 'Cannot Overlap',
                                              'Code Modifiers', 'Bachelors Degree', 'Less than Bachelors', 'Doctorate',
                                              'Same Code, Same Provider & Different POS',
                                              'Same Code, Different Provider & Different POS']
                                             )
    ...
    def read_npi(self):
        npi_table = self.excel.read_worksheet_as_table(self.get_real_sheet_name('CO Medicaid NPI #s'), True)
        for row in npi_table:
    ...
    def read_mapping_file(self):
        self.read_payor_and_fee_schedule()
        self.read_fee_schedule_rates()
        self.read_bcba_credential()
        self.read_fee_schedule_detail()
        self.read_sca()
        self.read_npi()

        self.excel.close_workbook()

    def read_crosswalk_guide(self):
        crosswalk_guide = self.excel.read_worksheet_as_table(self.get_real_sheet_name('Sheet1'), True)

        crosswalk_guide_dict: dict = {}
        for row in crosswalk_guide:
            hcpc_code: str = str(row['HCPC code']).replace('.0', '').strip().upper()
            cpt_code: str = str(row['CPT code']).replace('.0', '').strip().upper()
            if hcpc_code not in crosswalk_guide_dict:
                crosswalk_guide_dict[hcpc_code] = []
            if cpt_code not in crosswalk_guide_dict:
                crosswalk_guide_dict[cpt_code] = []

            crosswalk_guide_dict[hcpc_code].append({
                'code': cpt_code,
                'conversion': round(float(row['Conversion in Units']), 2),
            })
            crosswalk_guide_dict[cpt_code].append({
                'code': hcpc_code,
                'conversion': round(1 / float(row['Conversion in Units']), 2),
            })

        self.excel.close_workbook()
        return crosswalk_guide_dict
