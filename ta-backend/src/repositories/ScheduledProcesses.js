"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ScheduledProcessesRepository = void 0;
const typeorm_1 = require("typeorm");
const scheduled_processes_1 = require("@/@types/scheduled-processes");
const ScheduledProcess_1 = require("@/entities/ScheduledProcess");
let ScheduledProcessesRepository = class ScheduledProcessesRepository extends typeorm_1.AbstractRepository {
    constructor() {
        super(...arguments);
        this.insert = (scheduledProcess) => this.repository.save(scheduledProcess);
        this.getById = (id) => this.repository
            .createQueryBuilder('scheduledProcesses')
            .leftJoinAndSelect('scheduledProcesses.user', 'users')
            .leftJoinAndSelect('scheduledProcesses.process', 'processes')
            .where('scheduledProcesses.id = :id', { id })
            .getOne();
        this.getAllScheduled = (organizationId, userId) => {
            const query = this.repository
                .createQueryBuilder('scheduledProcesses')
                .leftJoinAndSelect('scheduledProcesses.process', 'processes')
                .leftJoinAndSelect('scheduledProcesses.user', 'users')
                .leftJoinAndSelect('scheduledProcesses.organization', 'organizations');
            if (organizationId) {
                query.andWhere('scheduledProcesses.organization.id = :id', { id: organizationId });
            }
            if (userId) {
                query.andWhere('scheduledProcesses.user.id = :id', { id: userId });
            }
            query.andWhere('scheduledProcesses.status = :status', {
                status: scheduled_processes_1.SCHEDULED_PROCESS_STATUS.SCHEDULED,
            });
            return query.getMany();
        };
        this.updateStatusById = (id, status) => this.repository
            .createQueryBuilder()
            .update()
            .set({ status })
            .where('id = :id', { id })
            .execute();
    }
};
ScheduledProcessesRepository = __decorate([
    typeorm_1.EntityRepository(ScheduledProcess_1.ScheduledProcess)
], ScheduledProcessesRepository);
exports.ScheduledProcessesRepository = ScheduledProcessesRepository;
//# sourceMappingURL=ScheduledProcesses.js.map