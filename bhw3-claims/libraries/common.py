...
def wait_element_is_exit(browser: Selenium, locator: str, timeout: int = 90):
    is_success = False
    timer = datetime.datetime.now() + datetime.timedelta(0, timeout)
        ...
        if not is_success:
            if browser.does_page_contain_element(
                ...
                if elem.is_displayed():
                    break
    if not is_success:
        raise Exception('Element \'{}\' not available'.format(locator))
...
def convert_json_file_to_dictionary(path_to_json):
