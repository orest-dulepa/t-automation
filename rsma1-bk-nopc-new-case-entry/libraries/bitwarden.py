...
class Bitwarden:
    def __init__(self, credentials: dict):
        ...
        if platform.system().lower() != 'windows':
            subprocess.run(['chmod', '+x', self.path_to_exe_file], capture_output=True, text=True)
        if platform.system().lower() == 'darwin':
            subprocess.run(['sudo', self.path_to_exe_file, 'logout'], capture_output=True, text=True)
        else:
