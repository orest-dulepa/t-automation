...
class GoogleSheets(GoogleServices):
    ...
    spreadsheet_id: str = ''
    sheet_name: str = "'AMBilling Invoice Requests'!"
    ...
    def get_sheet_data(self, spreadsheet_id: str) -> list:
        ...
        data = self.service.spreadsheets().values().batchGet(
            ...
            ranges=f"{self.sheet_name}A1:L1000"
        ).execute()
        rows: list = data['valueRanges'][0]['values']

        return rows

    def write_to_sheet(self, column_name: str, index_row: int, value: list) -> None:
        self.service.spreadsheets().values().update(
            spreadsheetId=self.spreadsheet_id,
            range=self.sheet_name + self.columns[column_name.strip().lower()] + str(index_row),
